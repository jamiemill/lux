...
var CONTROL_TEMPLATE_FILE = __dirname + '/views/control.jade';
var CONTROL_ASSETS_TEMPLATE_FILE = __dirname + '/views/control-window-assets.jade';
...
function serveControlPage(req, res) {
    deferred.map([CONTROL_TEMPLATE_FILE, CONTROL_ASSETS_TEMPLATE_FILE], function(fileName) {
        return readFile(fileName, 'utf-8');
    })
    .then(function(files) {
        var template = jade.compile(files[0], {filename: TEMPLATE_FILE});
...
function startServer() {
    ...
    app.get('/control', serveControlPage);
